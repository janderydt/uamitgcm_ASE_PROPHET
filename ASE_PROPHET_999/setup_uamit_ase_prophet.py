...
root='/media/wchm8/mainJDRydt2/UaMITgcm_v2/'
sys.path.append(root+'/UaMITgcm_archer2/tools/')
sys.path.append(root+'/UaMITgcm_archer2/coupling/')
...
obcs_forcing_data = 'Naughten_baseline' # either 'Kimura', 'Holland' or 'Naughten_paris2c', 'Naughten_rcp85', 'Naughten_baseline'
...
class OBCSForcingArray:
    ...
    def __init__ (self):
        ...
        if obcs_forcing_data == 'Kimura':
            ...
            self.BC = loadmat(root+'/MIT_InputData/PTDC_999/Kimura_OceanBC.mat')
            self.prefix = ''
        elif obcs_forcing_data == 'Holland':
            print('Using Holland data for obcs conditions')
            self.BC = loadmat(root+'/MIT_InputData/PTDC_999/Holland_OceanBC.mat')
            self.prefix = '' 
        elif obcs_forcing_data == 'Naughten_paris2c':
            print('Using Naughten Paris2C data for obcs conditions')
            self.BC = loadmat(root+'/MIT_InputData/ASE_PROPHET_999/Naughten_OceanBC_paris2c.mat')
            self.prefix = 'paris2c'
        elif obcs_forcing_data == 'Naughten_rcp85':
            print('Using Naughten RCP85 data for obcs conditions')
            self.BC = loadmat(root+'/MIT_InputData/ASE_PROPHET_999/Naughten_OceanBC_rcp85.mat')
            self.prefix = 'rcp85'
        elif obcs_forcing_data == 'Naughten_baseline':
            print('Using Naughten Baseline data for obcs conditions')
            self.BC = loadmat(root+'/MIT_InputData/ASE_PROPHET_999/Naughten_OceanBC_baseline.mat')
...
# first we isolate the ocean spinup from the forcing dataset
        ...
        startIndex = int(np.intersect1d(BCyears,BCmonths))
...
def make_ics_obcs (grid, obcs, ini_temp_file, ini_salt_file, obcs_temp_file_S, obcs_salt_file_S, obcs_uvel_file_S, obcs_vvel_file_S, obcs_temp_file_W, obcs_salt_file_W, obcs_uvel_file_W, obcs_vvel_file_W, pload_file, prec):
    ...
    calc_load_anomaly(grid, pload_file, option='nearest', ini_temp_file=ini_temp_file, ini_salt_file=ini_salt_file, eosType=eosType, rhoConst=rhoConst, prec=prec, check_grid=False)
...
prefix = obcs.prefix
make_ics_obcs(grid, obcs, input_dir+'T_ini_'+prefix+'.bin', input_dir+'S_ini_'+prefix+'.bin', input_dir+'OBSt_'+prefix+'.bin', input_dir+'OBSs_'+prefix+'.bin', input_dir+'OBSu_'+prefix+'.bin', input_dir+'OBSv_'+prefix+'.bin', input_dir+'OBWt_'+prefix+'.bin', input_dir+'OBWs_'+prefix+'.bin', input_dir+'OBWu_'+prefix+'.bin', input_dir+'OBWv_'+prefix+'.bin', input_dir+'pload_'+prefix+'.mdjwf', prec=64)
