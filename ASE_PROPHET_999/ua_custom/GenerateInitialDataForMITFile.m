...
addpath(genpath("/media/wchm8/mainJDRydt2/UaMITgcm_v2/UaMITgcm_archer2/UaSource_beta_19Jan2024/"));
runID = "ASE_PROPHET_001";
...
froot = "/media/wchm8/mainJDRydt2/UaMITgcm_v2/Ua_InputData/";

RunTable=readtable(froot+"RunTable.csv") ;
I=find(strcmp({RunTable{:,'ID'}{:}},runID)) ;
%I = I(2); % only need row number
 
UserVar.NameOfFileForReadingSlipperinessEstimate = froot+RunTable{I,'Row'}{:}+"_C-Estimate.mat";
%if ~exist(UserVar.NameOfFileForReadingSlipperinessEstimate,"file")
    xC=MUA.coordinates(:,1); yC=MUA.coordinates(:,2); C=F.C; m=F.m; muk=F.muk; q=F.q; 
    save(UserVar.NameOfFileForReadingSlipperinessEstimate,"MUA","CtrlVarInRestartFile","xC","yC","C","m","muk","q");
%end

UserVar.NameOfFileForReadingAGlenEstimate = froot+RunTable{I,'Row'}{:}+"_AGlen-Estimate.mat";
%if ~exist(UserVar.NameOfFileForReadingAGlenEstimate,"file")
    xA=MUA.coordinates(:,1); yA=MUA.coordinates(:,2); AGlen=F.AGlen; n=F.n; 
save(UserVar.NameOfFileForReadingAGlenEstimate,"MUA","CtrlVarInRestartFile","xA","yA","AGlen","n");
%end
...
switch RunTable{I,'GeometryInterpolants'}{:}
    ...
    case 'Bedmachinev3_2000'
        UserVar.GeometryInterpolants = froot+"GriddedInterpolants_Geometry_01-Jun-2000_EXTRUDED.mat"; 
    otherwise
        error('Geometry does not exist');
end
%UserVar.FirnInterpolants = [froot,'GriddedInterpolants_Firn_RACMO.mat'];
UserVar.RACMO_SMB = froot+"ScatteredInterpolants_SMB.mat";
...
if ~exist('../ua_run/RefinedMesh_for_MITmask.mat')
    disp('Generating refined mask');
    ...
    save('../ua_run/RefinedMesh_for_MITmask.mat','MUA_new');
else
    load('../ua_run/RefinedMesh_for_MITmask.mat');
end
